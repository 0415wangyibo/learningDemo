package com.potoyang.learn.securityoauth2.configauto.jdbcdetail;
...
public class MyJdbcTokenStore implements TokenStore {
    ...
    public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
        ...
        try {
            accessToken = this.jdbcTemplate.queryForObject(this.selectAccessTokenFromAuthenticationSql, new RowMapper<OAuth2AccessToken>() {
                @Override
    ...
    public OAuth2AccessToken readAccessToken(String tokenValue) {
        ...
        try {
            ...
            accessToken = this.jdbcTemplate.queryForObject(this.selectAccessTokenSql, new RowMapper<OAuth2AccessToken>() {
                @Override
                public OAuth2AccessToken mapRow(ResultSet rs, int rowNum) throws SQLException {
                    return MyJdbcTokenStore.this.deserializeAccessToken(rs.getBytes(2));
                }
    ...
    public OAuth2Authentication readAuthentication(String token) {
        ...
        try {
            authentication = this.jdbcTemplate.queryForObject(this.selectAccessTokenAuthenticationSql, new RowMapper<OAuth2Authentication>() {
                @Override
...
/*        try {
    ...
    public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
        ...
        try {
            accessTokens = this.jdbcTemplate.query(this.selectAccessTokensFromClientIdSql, new SafeAccessTokenRowMapper(), clientId);
        } catch (EmptyResultDataAccessException var4) {
            if (LOG.isInfoEnabled()) {
    ...
    private final class SafeAccessTokenRowMapper implements RowMapper<OAuth2AccessToken> {
        ...
        public OAuth2AccessToken mapRow(ResultSet rs, int rowNum) throws SQLException {
            try {
                ...
                MyJdbcTokenStore.this.jdbcTemplate.update(MyJdbcTokenStore.this.deleteAccessTokenSql, token);
                return null;
            }
        }
    }
